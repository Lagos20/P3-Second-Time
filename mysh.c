...
char *SearchFullPath(const char *command) { 
    ...
    while (*start) {
        ...
        *end = '\0';
        ...
        snprintf(full_path, BUFSIZE, "%s/%s", start, command);
...
void expand_wildcards(char *token, char **args, int *arg_index) {  //change
    DIR *dir = opendir(".");
    if (!dir) {
        perror("opendir");
        return;
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (fnmatch(token, entry->d_name, 0) == 0) {  //consider change
...
void ExecuteCommand(char **args, int in_fd, int out_fd) {  //change
...
void ControlPipeline(char *line) { //change
    char *commands[2];
...
void RunTheShell(FILE *input, int interactive) { //change
